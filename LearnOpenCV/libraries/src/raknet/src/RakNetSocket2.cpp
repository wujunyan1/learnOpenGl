...
bool IRNS2_Berkley::IsPortInUse(unsigned short port, const char *hostAddress, unsigned short addressFamily, int type ) {
	...
	bbp.port=port; strcpy(bbp.hostAddress,(char*) hostAddress);	bbp.addressFamily=addressFamily;
...
void RNS2_Berkley::CheckTotalPacket()
{
	if (!m_needCheckDDOS)
		return;

	if (m_lastCheckTotalPacketTime == 0)
		return;

	int invalidPeer = 0;
	if (RakNet::GetTimeMS() - m_lastCheckTotalPacketTime >= 1000)
	{
		for (auto& packet : m_packetStatics)
		{
			if (packet.second > m_totalPacketsPerSec[packet.first])
			{
				m_totalPacketsPerSec[packet.first] = packet.second;
				auto addr = m_addrHashToAddr[packet.first];

				auto invalidPackets = binding.eventHandler->getInvalidPacketCount(addr);
				binding.eventHandler->onUpdatePacketStatics(addr, packet.second, invalidPackets);

				//如果每秒收包超过阈值（默认100)而且都是非法包 拉入黑名单
				if (packet.second > m_invalidPacketThreshold && invalidPackets> 1)
				{
					invalidPeer++;
					m_blackAddrs[packet.first] = 1;
					binding.eventHandler->onUpdateBlackListCount(m_blackAddrs.size());
					if(invalidPeer > 50)
					{//1s有100个人出现这种情况 大概率就是被攻击了，开启严格模式只要是收到超过10个非法包就拉黑名单
						m_invalidPacketThreshold = 10;
					}
		...
		m_lastCheckTotalPacketTime = RakNet::GetTimeMS();
	}
}
...
unsigned RNS2_Berkley::RecvFromLoopInt(void)
{
	...
	while ( endThreads == false )
	{
		...
		if (recvFromStruct != NULL)
		{
			...
			auto addrHash = RakNet::SystemAddress::ToInteger(recvFromStruct->systemAddress);
			auto isInBlackList = m_blackAddrs.find(addrHash) != m_blackAddrs.end();
			if (recvFromStruct->bytesRead>0 && !isInBlackList)
			{
				if (m_lastCheckTotalPacketTime == 0)
					m_lastCheckTotalPacketTime = RakNet::GetTimeMS(); //收到第一个包开始统计

				RakAssert(recvFromStruct->systemAddress.GetPort());
				binding.eventHandler->OnRNS2Recv(recvFromStruct);
				
				m_packetStatics[addrHash] += 1;
				m_addrHashToAddr[addrHash] = recvFromStruct->systemAddress;
			}
			else
			{
				RakSleep(0);
				binding.eventHandler->DeallocRNS2RecvStruct(recvFromStruct, _FILE_AND_LINE_);
				if (m_needRebindSocket)
				{
					this->rebindSocket();
					m_needRebindSocket = false;
				}
			}
		}
	}
...
void RNS2_Berkley::markNeedCheckDDOS()
{
	m_needCheckDDOS = true;
}

void RNS2_Berkley::markRebindSocket()
{
	m_needRebindSocket = true;
}

void RNS2_Berkley::rebindSocket()
{
	RNS2BindResult rt = RNS2BindResult::BR_FAILED_TO_BIND_SOCKET;
	RNS2_BerkleyBindParameters bbp;
	bbp = *GetBindings();
	int tryTimes = 0;
	while (rt == RNS2BindResult::BR_FAILED_TO_BIND_SOCKET && tryTimes < 10)
	{
		RakSleep(100);
		rt = Bind(&bbp, __FILE__, __LINE__);
		RAKNET_DEBUG_PRINTF("begin rebind socket");
		tryTimes++;
	}
}
...
void RNS2_Berkley::BlockOnStopRecvPollingThread(void)
{
	...
	RakNet::TimeMS timeout = RakNet::GetTimeMS()+3000;
