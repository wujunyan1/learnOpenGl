...
static const int mtuSizes[NUM_MTU_SIZES]={MAXIMUM_MTU_SIZE, 576, 576};
	...
	for (i=0; i<socketDescriptorCount; i++)
	{
		...
		*/
		...
		if (r2->IsBerkleySocket())
		{
			...
			strcpy(bbp.hostAddress, socketDescriptors[i].hostAddress);
			...
			if (maxConnections > 1)
				((RNS2_Berkley*)r2)->markNeedCheckDDOS();
			if (
...
void RakPeer::ChangeAddressForConnection(const RakNetGUID &guid, const SystemAddress &newSystemAddress)
{
	auto remoteSystem = this->GetRemoteSystemFromGUID(guid, false);
	if (remoteSystem && remoteSystem->systemAddress != newSystemAddress)
	{
		auto saveIndex = remoteSystem->systemAddress.systemIndex;
		remoteSystem->systemAddress = newSystemAddress;
		remoteSystem->systemAddress.systemIndex = saveIndex;
	}
}
...
uint64_t RakPeerInterface::Get64BitUniqueRandomNumber(void)
{
	...
	return (uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(tt.time_since_epoch()).count();
...
/*
	...
	auto tt = nowTime.time_since_epoch().count();
#else
	struct timeval tv;
...
/*
		...
		(size_t) length >= sizeof(MessageID) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID))
	{
		*isOfflineMessage=memcmp(data+sizeof(MessageID) + RakNetGUID::size(), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID))==0;
	    ...
	    (unsigned char)data[0] == ID_REQUEST_GUID||
		(unsigned char)data[0] == ID_SEND_GUID) &&
		(size_t) length >= sizeof(MessageID) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID))
	{
	...
	if (*isOfflineMessage)
	{
		...
		else if ((unsigned char)(data)[0] == ID_REQUEST_GUID)
		{
			RakNet::BitStream bsOut;
			bsOut.Write((MessageID)ID_SEND_GUID);
			bsOut.WriteAlignedBytes((const unsigned char*)OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID));
			bsOut.Write(rakPeer->GetMyGUID());

			RNS2_SendParameters bsp;
			bsp.data = (char*)bsOut.GetData();
			bsp.length = bsOut.GetNumberOfBytesUsed();
			bsp.systemAddress = systemAddress;
			RAKNET_DEBUG_PRINTF("send ID REQUEST GUID");
			rakNetSocket->Send(&bsp, _FILE_AND_LINE_);

		}
		else if ((unsigned char)(data)[0] == ID_SEND_GUID)
		{
			RakNet::BitStream bsIn((unsigned char*)data, length, false);
			bsIn.IgnoreBytes(sizeof(MessageID));
			bsIn.IgnoreBytes(sizeof(OFFLINE_MESSAGE_DATA_ID));
			RakNetGUID clientGUID;
			bsIn.Read(clientGUID);
			rakPeer->ChangeSystemAddress(clientGUID, systemAddress);
			rakPeer->removeInvalidPacketLog(systemAddress);
		}
...
//	RakNet::Packet *packet;
	...
	{
		RakNet::BitStream bsOut;
		bsOut.Write((MessageID)ID_REQUEST_GUID);
		bsOut.WriteAlignedBytes((const unsigned char*)OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID));
		bsOut.Write(rakPeer->GetMyGUID());
		...
		bsp.systemAddress = systemAddress;
		rakNetSocket->Send(&bsp, _FILE_AND_LINE_);
		rakPeer->OnRecvInvalidPacket(systemAddress);
	}
}
...
//					SocketLayer::SetDoNotFragment(socketToUse, 1);
	...
	//	remoteSystemList[ remoteSystemIndex ].allowSystemAddressAssigment=true;
			...
			if (timeMS > remoteSystem->lastReliableSend && timeMS-remoteSystem->lastReliableSend > heatBeatTime && remoteSystem->connectMode==RemoteSystemStruct::CONNECTED)
			{
			...
			bool hasMarkToDead = remoteSystem->reliabilityLayer.IsDeadConnection();
			bool inDisconectStep = (
			...
			{
				RAKNET_DEBUG_PRINTF("timeMS=%llu remoteSystem->connectionTime=%llu, connectMode=%i\n", timeMS, remoteSystem->connectionTime, remoteSystem->connectMode);
...
//					memcpy(packet->data+1, undeliveredMessages.GetData(), undeliveredMessages.GetNumberOfBytesUsed());
			...
			if (isClientMode && remoteSystem->connectMode == RemoteSystemStruct::CONNECTED && (timeMS - remoteSystem->reliabilityLayer.GetTimeLastDatagramArrived() > needRebindTimeout))
			{
				if (lastRebindTime == 0 || timeMS - lastRebindTime > rebindRetryTime)
				{
					((RNS2_Berkley*)remoteSystem->rakNetSocket)->markRebindSocket();
					lastRebindTime = timeMS;
				}
			}
...
//								SystemAddress internalID;
								...
								outBitStream.Write(RakNet::GetTime());
								//ÔÝÊ±ÏÈ×ßchannnel 1
								SendImmediate( (char*)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 1, systemAddress, false, false, RakNet::GetTimeUS(), 0 );
...
void  RakPeer::OnRecvInvalidPacket(const SystemAddress& addr)
{
	InvalidPacketsMutex.Lock();
	m_recvInvalidPackets[SystemAddress::ToInteger(addr)] += 1;
	InvalidPacketsMutex.Unlock();
}


void RakPeer::removeInvalidPacketLog(const RakNet::SystemAddress& addr)
{
	InvalidPacketsMutex.Lock();
	auto removeIt = m_recvInvalidPackets.find(SystemAddress::ToInteger(addr));
	if(removeIt!= m_recvInvalidPackets.end())
		m_recvInvalidPackets.erase(removeIt);
	InvalidPacketsMutex.Unlock();
}

int  RakPeer::getInvalidPacketCount(const RakNet::SystemAddress& addr)
{
	InvalidPacketsMutex.Lock();
	auto packetCount = 0;
	auto checkIt = m_recvInvalidPackets.find(SystemAddress::ToInteger(addr));
	if (checkIt != m_recvInvalidPackets.end())
		packetCount = checkIt->second;
	InvalidPacketsMutex.Unlock();
	return packetCount;
}

void RakPeer::onUpdateBlackListCount(int count)
{
	m_blackListCount = count;
}

std::pair<int, int> RakPeer::getPacketCountStatics(const RakNet::SystemAddress& addr)
{
	auto rt = std::pair<int, int>({ 0, 0 });
	packetStaticsMutex.Lock();
	auto guid = SystemAddress::ToInteger(addr);
	if (m_totalPacketsPerSec.find(guid) != m_totalPacketsPerSec.end())
	{
		auto statics = m_totalPacketsPerSec.find(guid)->second;
		rt.second = statics & 0x00000000ffffffff;
		rt.first = (statics - rt.second) >> 32;
	}
	packetStaticsMutex.Unlock();
	return rt;
}


void RakPeer::onUpdatePacketStatics(const RakNet::SystemAddress& addr, int packets, int invalidPackets)
{
	packetStaticsMutex.Lock();
	auto guid = SystemAddress::ToInteger(addr);
	uint64_t statics = packets;
	statics = statics << 32;
	statics = statics  + invalidPackets;
	m_totalPacketsPerSec[guid] = statics;
	packetStaticsMutex.Unlock();
}
...
std::string RakPeer::netDataToHex(const void* data, int len)
{
	std::string rt;
	for (int i = 0; i < len; i++)
	{
...
void RakPeer::BMLog(const char* format, ...)
{
	if (bmWriter)
	{
