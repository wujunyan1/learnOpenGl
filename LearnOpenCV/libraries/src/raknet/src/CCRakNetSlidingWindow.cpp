...
void CCRakNetSlidingWindow::Init(CCTimeType curTime, uint32_t maxDatagramPayload)
{
	...
	cwnd= MIN_WIN *maxDatagramPayload;
	ssThresh=0.0;
	oldestUnsentAck=0;
...
void CCRakNetSlidingWindow::OnResend(CCTimeType curTime, RakNet::TimeUS nextActionTime)
{
	...
	allResendPacketThisTick += 1;
	if (_isContinuousSend  && allResendPacketThisTick >=3 && backoffThisBlock==false && cwnd>MAXIMUM_MTU_INCLUDING_UDP_HEADER*MIN_WIN)
	{
		...
		if (ssThresh == 0)
			...
			ssThresh = cwnd - MAXIMUM_MTU_INCLUDING_UDP_HEADER;

		if (ssThresh< MIN_WIN*MAXIMUM_MTU_INCLUDING_UDP_HEADER)
			ssThresh= MIN_WIN *MAXIMUM_MTU_INCLUDING_UDP_HEADER;
...
void CCRakNetSlidingWindow::OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond _B, BytesPerMicrosecond _AS, double totalUserDataBytesAcked, bool isContinuousSend, DatagramSequenceNumberType sequenceNumber )
{
	...
	else
	{
		...
		estimatedRTT = (7*estimatedRTT + rtt)/8;
		deviationRtt = (3*deviationRtt + difference)/4;
	}
...
CCTimeType CCRakNetSlidingWindow::GetRTOForRetransmission(unsigned char timesSent) const
{
	...
	CCTimeType threshhold = (CCTimeType)(u * estimatedRTT + q * deviationRtt) *1.5;
	if (threshhold < additionalVariance)
		return additionalVariance;
	else if (threshhold > maxThreshold)
		return maxThreshold;
	else
		return threshhold;
}
