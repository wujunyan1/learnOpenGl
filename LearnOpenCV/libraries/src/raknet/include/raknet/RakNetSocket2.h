...
public:
	...
	virtual void onUpdatePacketStatics(const RakNet::SystemAddress& addr, int packets, int invalidPackets) {};
	virtual int  getInvalidPacketCount(const RakNet::SystemAddress& addr) { return 0; };
	virtual void onUpdateBlackListCount(int count) {};
...
{
	// Input parameters
	RNS2_BerkleyBindParameters() { memset(hostAddress, 0, RAK_IP_LEN); }
	unsigned short port;
	char hostAddress[RAK_IP_LEN];
...
public:
	...
	void rebindSocket();
	void markRebindSocket();
	void markNeedCheckDDOS();
...
protected:
	...
	SimpleMutex useSocketLock;
	std::atomic_bool m_needRebindSocket{false};
	std::atomic_bool m_needCheckDDOS{ false };
	std::atomic_bool endThreads;

	std::map<unsigned long, int> m_packetStatics;
	std::map<unsigned long, int> m_totalPacketsPerSec;
	std::map<unsigned long, int> m_blackAddrs;
	std::map<unsigned long, SystemAddress> m_addrHashToAddr;

	int m_invalidPacketThreshold = 300;

	RakNet::TimeMS  m_lastCheckTotalPacketTime = 0;
