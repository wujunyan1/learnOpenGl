...
public:
	...
	void setClientMode(bool isClient) override{ isClientMode = isClient; }

	void setLogWriter(LogWriter func) override { bmWriter = func; }
	static void BMLog(const char* format, ...);
...
//		int connectionSocketIndex; // index into connectionSockets to send back on.
		RakNetGUID guid; // 这个是客户端本地生成后上报上来的 guid. 链接的时候会做guid重复性校验，所以可以保证唯一。
		int MTUSize;
...
protected:
	...
	void ChangeAddressForConnection(const RakNetGUID& guid, const SystemAddress& newSystemAddress);
...
//		char remote_challenge[cat::EasyHandshake::CHALLENGE_BYTES];
	...
	void  OnRecvInvalidPacket(const SystemAddress& addr);
	...
	RakNet::TimeMS lastRebindTime = 0;
	RakNet::TimeMS rebindRetryTime = 3000;
	RakNet::TimeMS needRebindTimeout = 6000;
	RakNet::TimeMS heatBeatTime = 2000;
	...
	SimpleMutex packetStaticsMutex;
	using PacketStaticsType = std::map<uint64_t, uint64_t>;
	PacketStaticsType m_totalPacketsPerSec;
	

	SimpleMutex InvalidPacketsMutex;
	PacketStaticsType m_recvInvalidPackets;
	int m_blackListCount = 0;

	static LogWriter bmWriter;
	...
	void onUpdatePacketStatics(const RakNet::SystemAddress& addr, int packets, int invalidPackets) override;
	int  getInvalidPacketCount(const RakNet::SystemAddress& addr) override;
	void onUpdateBlackListCount(int count) override;
	void removeInvalidPacketLog(const RakNet::SystemAddress& addr);
	std::pair<int, int> getPacketCountStatics(const RakNet::SystemAddress& addr) override;
	int getBlackListCount() override { return m_blackListCount; } ;
