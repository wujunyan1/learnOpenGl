...
#ifndef __LORD_QUAN_H__
#define __LORD_QUAN_H__
...
namespace LORD
{
	...
	\brief Quaternion
	*/
	class LORD_CORE_API Quaternion
	{
	public:
		union
		{
	...
	public:
		...
		{
			LordAssertX(pf != NULL, "The array data is NULL.");
			ADD_MATH_OP_COUNT
				w = pf[0];
			x = pf[1];
			y = pf[2];
		...
		inline Quaternion(const btQuaternion& other)
		...
		{
			ADD_MATH_OP_COUNT
			LordAssertX(w >= -1.0 && w <= 1.0, "The quaternion is not identity.");	
			return Math::ACos(w) * 2.0f;
		}

		inline void rotateAxis(const Vector3& axis, Real radian)
		{
			ADD_MATH_OP_COUNT
